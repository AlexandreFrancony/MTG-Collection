  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[var(--text-muted)]">Search the Scryfall database for cards to add to your collection.</p>
      </div>
      ...
      <form onSubmit={handleSearch} className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 bg-[var(--bg-card)] border border-[var(--border)] rounded-lg focus:outline-none focus:ring-2 focus:ring-mtg-gold"
          />
        </div>
        <button
          type="submit"
          disabled={loading}
          className="px-6 py-3 bg-mtg-gold text-[var(--bg-primary)] font-medium rounded-lg hover:bg-yellow-400 disabled:opacity-50 transition-colors"
        >
      ...
      {results.length > 0 && (
        <div>
          <p className="text-[var(--text-muted)] mb-4">{results.length} cards found</p>
                  ...
                  <button
                    ...
                    className={`mt-2 w-full py-1.5 rounded flex items-center justify-center gap-1 text-sm font-medium transition-colors ${
                      addedCards.has(card.scryfall_id)
                        ...
                        : 'bg-mtg-gold text-[var(--bg-primary)] hover:bg-yellow-400'
                    }`}
                  >
                    {addedCards.has(card.scryfall_id) ? (
                      <>
