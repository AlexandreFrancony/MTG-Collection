  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-lg p-4">
        <h3 className="font-medium text-[var(--text-secondary)] mb-2 flex items-center gap-2">
          <FileText size={18} />
          Supported formats
        </h3>
        <ul className="text-sm text-[var(--text-muted)] space-y-1">
          <li><code className="bg-[var(--bg-input)] px-1 rounded">4x Lightning Bolt</code> - quantity with 'x'</li>
          <li><code className="bg-[var(--bg-input)] px-1 rounded">4 Lightning Bolt</code> - quantity with space</li>
          <li><code className="bg-[var(--bg-input)] px-1 rounded">Lightning Bolt</code> - defaults to 1 copy</li>
        </ul>
      ...
      <form onSubmit={handleImport} className="space-y-4">
        <div>
          ...
          <textarea
            ...
            rows={12}
            className="w-full px-4 py-3 bg-[var(--bg-card)] border border-[var(--border)] rounded-lg focus:outline-none focus:ring-2 focus:ring-mtg-gold font-mono text-sm resize-y"
          />
        ...
        <button
          ...
          className="w-full sm:w-auto px-6 py-3 bg-mtg-gold text-[var(--bg-primary)] font-medium rounded-lg hover:bg-yellow-400 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
        >
          {loading ? (
            <>
      ...
      {result && (
        <div className="space-y-6">
          {/* Summary */}
          <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-lg p-4">
            <h3 className="font-medium text-[var(--text-secondary)] mb-3">Import Summary</h3>
          ...
          {result.added.length > 0 && (
            <div>
              <h3 className="font-medium text-[var(--text-secondary)] mb-3 flex items-center gap-2">
                ...
                {result.added.map((card) => (
                  <div key={card.id} className="relative group">
                    ...
                    <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/90 to-transparent p-2">
                      <p className="text-xs font-medium truncate text-white">{card.card_name}</p>
                      <p className="text-xs text-[var(--text-muted)]">x{card.quantity}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                {result.updated.map((item) => (
                  <div key={item.card.id} className="relative group">
                    ...
                    />
                    <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/90 to-transparent p-2">
                      <p className="text-xs font-medium truncate text-white">{item.card.card_name}</p>
